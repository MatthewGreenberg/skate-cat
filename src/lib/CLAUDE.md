...
- **leaderboard.js** — Top-10 leaderboard client. `fetchLeaderboard()` (async, `GET /api/leaderboard`), `submitScore(initials, score, rank)` (async, `POST /api/submit-score`, returns refreshed board), `isHighScore(score, board)` (pure — pass the cached board from React state). Browser never hits Supabase directly; the service-role key lives only on Vercel.
